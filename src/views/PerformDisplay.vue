          ...
          <div class="modal-content-inner">
            <div class="modal-content-body" ref="modalContentBodyRef">
              ...
              <ul class="competi-list nolink" >
                <li v-for="data in dataList" :key="data.snno" @click="selectPerformDetailClick($event,data.snno)" @touchend="selectPerformDetailClick($event,data.snno)" @touchmove="selectTouchClick">
                  <div class="competi-pic"><img :src="data.titleImg" alt="" /></div>
                  <div class="competi-info-outer">
                    <strong>{{data.title}}</strong>
                    <ul>
                      <li><span>제공 :</span> {{data.provd}}</li>
...
export default {
  ...
  setup(props,context){
    ...
    const connectInitFunc = async (pSaveStatsCd)=> {
      ...
      dataList.value=storeData.state.dataStore.pblprfrDspyStateList;
    ...
    //행사축데목록에서 상세를 클릭했을경우
    const selectPerformDetailClick =(event,pSnno)=>{
      event.preventDefault();
