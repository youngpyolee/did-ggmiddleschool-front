<template>
  <div data-schtab-content="2" class="active">
    <div class="search-sch">
      <div>
        <ul class="select-item subside gg">
          <li :class="{active:upperData.upperSubjectCd==searchUpperSubjectCd, gg:true}" v-for="upperData in upperSubjectList" @click="upperSubjectListClick($event, upperData.upperSubjectCd)" @touchend="upperSubjectListClick($event, upperData.upperSubjectCd)" :key="upperData.upperSubjectCd">{{upperData.upperSubjectNm}}</li>
        </ul>
      </div>
      <div>
  ...
  setup(props,context) {
    ...
    const selectHighschoolSelectedClick = async (event,schulCd)=>{
      if (isScrolling==false){
        let sendParam={"schulCd":schulCd,"searchDetailInfoType":"DB","searchInfoPositionType":"SchoolAreaInfoSubject"}
