<template>
  <div class="content-outer">
    ...
    <div class="icon-grid">
      ...
      <div class="grid-item" v-for="(iconData,sindex) in iconList" :key="iconData.subIndex" @touchend="selectIconClick(iconData)">
    ...
    <teleport to="#modalPopUp">
      ...
      <TestDiagnosisComponent     v-show="$store.state.modalPopStore.testDiagnosisState==true"   ref="testDiagnosisRef"  />
    </teleport>
  </div>
</template>
<script>
...
import TestDiagnosisComponent from "@/components/TestDiagnosisPopComponent";
...
export default {
  ...
  setup(props){
    ...
    onBeforeMount(async ()=>{
      console.log("=== IconViewList onBeforeMount START ===");
      ...
      console.log("=== IconViewList onBeforeMount END ===");
    });

    onMounted(()=>{
      console.log("=== TestDiagnosis onMounted START ===");
      ...
      console.log("=== TestDiagnosis onMounted END ===");
    });

    onUnmounted(()=>{
      console.log("=== TestDiagnosis onUnmounted START ===");
      randomIndex.value=0;
      console.log("=== TestDiagnosis onUnmounted END ===");
    });

  /*  const selectIconClick = (iconData)=>{
      storeData.dispatch("modalPopStore/modalProcStateAction" , {"stateNm":"iconPopState","stateYn":true});
      testDiagnosisPopRef.value.connectInitFunc(iconData);
    }*/

    //아이콘을 클릭했을경우
    const selectIconClick =(pIconData)=>{
      ...
      CommonUtil.staticAccess(CONFIG.STATIC_URL,storeData.state.configStore.didCd,storeData.state.configStore.didEqpmnCd,pIconData.statisticCd);
    }
