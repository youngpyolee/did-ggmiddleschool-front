          ...
          <div class="modal-content-inner">
            ...
            <div class="modal-content-header">
              ...
              <div class="modal-tabmenu">
                <div :class="{active:searchDetailInfoType=='DA'}"  @click="searchDetailInfoTypeSelectClick($event,'DA')" @touchend="searchDetailInfoTypeSelectClick($event,'DA')"><a href="javascript:void(0)">학교정보</a></div>
                <div :class="{active:searchDetailInfoType=='DB'}" v-show='displaySubjectImgNm !=""'  @click="searchDetailInfoTypeSelectClick($event,'DB')" @touchend="searchDetailInfoTypeSelectClick($event,'DB')">
                  <a href="javascript:void(0)">학과정보</a>
                </div>
                <!-- <div :class="{active:searchDetailInfoType=='DC'}"  @click="searchDetailInfoTypeSelectClick($event,'DC')" @touchend="searchDetailInfoTypeSelectClick($event,'DC')"><a href="javascript:void(0)">학교앨범</a></div> -->
              </div>
            </div>
            <div class="modal-content-body" ref="modalContentBodyRef">
              <div class="schInfo-outer" data-schinfo-tab="1" v-show="searchDetailInfoType=='DA'">
              ...
              <!-- 학과정보 상세 -->
              <div style="text-align:center; padding-top:40px;" v-if='searchDetailInfoType=="DB" && displaySubjectImgNm !=""'>
                <img :src="displaySubjectImgNm" alt="" style="max-width: 100%">
              </div>

              <!-- 학교 앨범 -->
<!--              <div class="pic-grid" v-if='searchDetailInfoType=="DC"'>
    ...
    <teleport to="#modalPopUp">
      <!-- 학교앨범상세 start -->
      <SchoolAreaInfoAlbum v-show="$store.state.modalPopStore.schoolPopAlbumState==true" ref="schoolAreaInfoAlbumRef" @close="albumPopCloseEvent" />
      <!-- 학교앨범상세 end  -->
    </teleport>
  </div>
</template>
...
import SchoolAreaInfoAlbum from "@/views/SchoolAreaInfoAlbum";
export default {
  name: "SchoolAreaInfoDetail",
  components: {
    SchoolAreaInfoAlbum,
  },
  setup(props,context) {
    ...
    let searchDetailInfoType=ref(""); //학교정보, 학과정보, 앨범인지 구분
    ...
    let schoolAreaInfoAlbumRef=ref();
    ...
    */
     ...
     * @param pSchulCd
     * @param psearchDetailInfoType
     * @param pSearchInfoPositionType
     * @param pSaveStatsCd
     * @returns {Promise<void>}
     */
    const connectInitFunc = async (pSchulCd,pSearchDetailInfoType,pSearchInfoPositionType,pSaveStatsCd)=>{
      searchSchulCd.value=pSchulCd;
      searchDetailInfoType.value=pSearchDetailInfoType;
      ...
      if (searchDetailInfoType.value=="DA"){
        console.log("ssssssss")
        loadChart=drawChart();
      }

      //loadedmetadata
      console.log(disPlaySchulVidoNm.value)
      if (searchDetailInfoType.value=="DA" && disPlaySchulVidoNm.value !=""){
    ...
    //학교정보,학교앨범 학과정보, 클릭시
    const searchDetailInfoTypeSelectClick = async (event,psearchDetailInfoType)=>{
      ...
      //학교정보를 클릭시
      if (psearchDetailInfoType=="DA" && disPlaySchulVidoNm.value !=""){
        videoPlay(true);
        setTimeout(()=>{
          loadChart=drawChart();
        },300);
      ...
      //학과정보 혹은 앨범클릭시
      else if (psearchDetailInfoType=="DB" || psearchDetailInfoType=="DC"){
        if (videoObj !=null){
          videoObj.pause();
        }else{
          //
    ...
    const selectFindSchoolInfoFunc = async (pSchulCd)=>{
      ...
      console.log("displayManStdntCnt.value="+displayManStdntCnt.value)
    ...
    const selectAlbumClick= async (albumData)=>{
      ...
      let result = await  schoolAreaInfoAlbumRef.value.connectInitFunc(searchSchulCd.value,albumData.seq,albumData.noti,albumData.regDate,albumData.dataUrl);
    }
    ...
    return {
      ...
      displaySwmplCnt,
      displayCnsltCnt,
      searchDetailInfoTypeSelectClick,
      saveStatsCd,
      albumLit,
      modalContentBodyRef,
      selectAlbumClick,
      albumPopCloseEvent,
      schoolAreaInfoAlbumRef,
      displayManStdntCnt,
